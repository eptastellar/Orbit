...
import { AccessMiddleware, CronMiddleware, SessionMiddleware } from '@/common/middlewares';
...
export class AppModule {
  configure(consumer: MiddlewareConsumer) {
    consumer
      .apply(AccessMiddleware).forRoutes({ path: 'auth/sign-*', method: RequestMethod.ALL })
      .apply(CronMiddleware).forRoutes({ path: 'cron/*', method: RequestMethod.ALL })
      .apply(SessionMiddleware).forRoutes({ path: 'test', method: RequestMethod.ALL }) //TODO
  }
}
